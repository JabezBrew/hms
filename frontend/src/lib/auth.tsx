...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const [facilityCode, setFacilityCodeState] = useState<string | null>(null)
  const [mfaSession, setMfaSession] = useState<string | null>(null)
  const [mfaUser, setMfaUser] = useState<LoginResponseUser | null>(null)
  const [mfaEnrollmentRequired, setMfaEnrollmentRequired] = useState(false)
  const [mfaAvailableMethods, setMfaAvailableMethods] = useState<MfaMetadata | null>(null)
  ...
  const setFacilityCode = useCallback((code: string | null) => {
  ...
  const logout = useCallback(async (localOnly = false): Promise<void> => {
    try {
      if (!localOnly) {
        try {
  ...
  const handleRefreshFailure = useCallback(async (): Promise<void> => {
  ...
  // Login function
  const applyAuthResponse = useCallback((response: LoginResponse | null | undefined): AuthUser | null => {
  ...
  ): Promise<AuthUser | { mfaRequired: true } | null> => {
    setError(null)
    try {
      // Call the login API
      const response = (await authApi.login(email, password, facility)) as LoginResponse
  ...
  const completeMfa = useCallback((response: LoginResponse): AuthUser | null => applyAuthResponse(response), [applyAuthResponse])
