...
import { useScheduleSlots, useScheduleTemplate } from '@/features/appointments/hooks/useAppointmentQueries';
...
const PractitionerAvailabilityDetailPage = () => {
  ...
  const pageMeta = usePageMeta({
    title: template?.name
      ? `${template.name} | Hospital Management System`
      : 'Schedule Template | Hospital Management System',
    breadcrumbs: [
      { label: 'Availability', path: '/practitioner-availability' },
      { label: template?.name || 'Schedule', path: `/practitioner-availability/${id}` },
    ],
  });
  ...
  const handleDelete = async () => {
    ...
    try {
      await appointmentsApi.deleteTimeSlot(timeSlotToDelete.id);
    ...
    return (
      <>
        ...
        <PageState
          ...
          description="The requested schedule template could not be found."
          action={
  ...
  return (
    <PageShell>
      ...
      <PageHeader
        title={template.name}
        description="Define when the practitioner is available for appointments."
        actions={(
