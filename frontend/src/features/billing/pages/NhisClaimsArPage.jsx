  ...
  const batchColumns = useMemo(() => ([
    ...
    },
    {
      ...
      render: (row) => {
        ...
        const meta = map[row.status] || { label: row.status, cls: 'bg-muted text-muted-foreground', icon: Clock };
  ...
  const exportColumns = useMemo(() => ([
    ...
    },
    {
      ...
      render: (row) => {
        ...
        }[row.status] || { cls: 'bg-muted text-muted-foreground', label: row.status, icon: Clock };
        const Icon = meta.icon;
        return (
          <span className={cn('inline-flex items-center gap-1 font-mono text-xs px-2 py-1 rounded', meta.cls)}>
            <Icon className="size-3" />
      ...
      <main className="p-4 sm:p-6">
        <Tabs value={tab} onValueChange={setTab}>
          ...
          <TabsContent value="batches" className="mt-6 space-y-6">
            ...
            <section className="bg-card border border-border rounded-2xl p-5 sm:p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="font-display text-lg text-foreground">Batches</h2>
                <p className="font-mono text-xs text-muted-foreground">{batchesTotal} total</p>
              </div>

              {batches.length === 0 ? (
                <p className="text-sm text-muted-foreground">No batches found.</p>
              ) : (
                <VirtualizedTable rows={batches} columns={batchColumns} threshold={50} />
              )}

              {batchesPages > 1 && (
                <div className="flex items-center justify-between mt-4">
                  <p className="font-mono text-xs text-muted-foreground">
                    Page {batchPage} of {batchesPages}
                  </p>
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      size="sm"
                      className="font-mono text-xs"
                      disabled={!batchesQuery.data?.next}
          ...
          <TabsContent value="exports" className="mt-6 space-y-6">
            ...
            <section className="bg-card border border-border rounded-2xl p-5 sm:p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <p className="font-mono text-xs text-muted-foreground">{exportsTotal} total</p>
              </div>

              {exportJobs.length === 0 ? (
                <p className="text-sm text-muted-foreground">No export jobs found.</p>
              ) : (
                <VirtualizedTable rows={exportJobs} columns={exportColumns} threshold={50} />
              )}

              {exportsPages > 1 && (
                <div className="flex items-center justify-between mt-4">
                  <p className="font-mono text-xs text-muted-foreground">
                    Page {exportPage} of {exportsPages}
                  </p>
                  <div className="flex items-center gap-2">
                    <Button
                      variant="outline"
                    ...
                    <Button
                      ...
                      size="sm"
                      className="font-mono text-xs"
                      disabled={!exportsQuery.data?.next}
          ...
          <TabsContent value="remittances" className="mt-6 space-y-6">
            ...
            <section className="bg-card border border-border rounded-2xl p-5 sm:p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="font-display text-lg text-foreground">Remittance Imports</h2>
                <p className="font-mono text-xs text-muted-foreground">{remittancesTotal} total</p>
              </div>

              {remittanceJobs.length === 0 ? (
                <p className="text-sm text-muted-foreground">No remittance imports found.</p>
              ) : (
                <VirtualizedTable rows={remittanceJobs} columns={remittanceColumns} threshold={50} />
              )}

              {remittancesPages > 1 && (
                <div className="flex items-center justify-between mt-4">
                  <p className="font-mono text-xs text-muted-foreground">
                    Page {remittancePage} of {remittancesPages}
                  </p>
                  <div className="flex items-center gap-2">
                    <Button
                      variant="outline"
                    ...
                    <Button
                      ...
                      size="sm"
                      className="font-mono text-xs"
                      disabled={!remittancesQuery.data?.next}
            ...
            <section className="bg-card border border-border rounded-2xl p-5 sm:p-6">
              <div className="flex items-center justify-between gap-3 flex-col sm:flex-row">
                ...
                <div className="w-full sm:w-[260px]">
                  <Select value={arBasis} onValueChange={setArBasis}>
                    ...
                    <SelectContent>
                      <SelectItem value="invoice_date" className="font-mono text-sm">Basis: Invoice Date</SelectItem>
                      <SelectItem value="claim_submission_date" className="font-mono text-sm">Basis: Claim Submission</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </div>
              ...
              <div className="mt-5 grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <div className="rounded-2xl border border-border bg-muted/10 p-5 lg:col-span-2">
                  <p className="font-mono text-xs text-muted-foreground uppercase tracking-wider mb-2">Remittance Queue</p>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    {(queue.summary || []).length > 0 ? (
                      (queue.summary || []).map((row) => (
          ...
          <div className="space-y-3">
            {linesQuery.isLoading ? (
              ...
              <>
                ...
                {Math.ceil((linesQuery.data?.count || 0) / 50) > 1 && (
                  <div className="flex items-center justify-between">
                    <p className="font-mono text-xs text-muted-foreground">
                      Page {linesPage} of {Math.ceil((linesQuery.data?.count || 0) / 50)}
                    </p>
                    <div className="flex items-center gap-2">
                      <Button
                        variant="outline"
