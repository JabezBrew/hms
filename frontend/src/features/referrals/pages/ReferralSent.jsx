...
function SentReferralsLoadingState() {
  return (
    <PageShell>
      <PageHeader
...
function SentReferralsHeader({ referralCount, breachedCount }) {
  return (
    <PageHeader
      title="Sent Referrals"
      description="Track the status of referrals you've sent to specialists"
...
function SentReferralStatusCounts({ statusCounts }) {
  return (
...
function SentReferralSearch({ searchQuery, onSearchChange }) {
  return (
...
function SentReferralEmptyState({ hasActiveFilters }) {
  return (
...
function SentReferralBadges({ referral }) {
...
function SentReferralCardHeader({ referral, onViewDetails }) {
  return (
    <CardHeader>
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-2 flex-wrap">
            ...
            <SentReferralBadges referral={referral} />
          </div>
...
function SentReferralProgression({ referral }) {
...
function SentReferralStatusPanels({ referral }) {
  return (
    <>
...
function SentReferralCard({ referral, onViewDetails }) {
  return (
    <Card className="bg-card border-border animate-chronicle-enter">
      <SentReferralCardHeader referral={referral} onViewDetails={onViewDetails} />
      <SentReferralProgression referral={referral} />
      <CardContent className="space-y-3">
        ...
        <SentReferralStatusPanels referral={referral} />
...
function SentReferralList({ referrals, hasActiveFilters, onViewDetails }) {
  if (referrals.length === 0) {
    return <SentReferralEmptyState hasActiveFilters={hasActiveFilters} />;
  }
...
function ReferralDetailStatusBadges({ referral }) {
  return (
    <div className="flex items-center gap-2 flex-wrap">
      <span className={cn('text-xs px-2 py-0.5 rounded-full', statusConfig[referral.status]?.badgeClass)}>
        {statusConfig[referral.status]?.label}
      </span>
      <span className={cn('text-xs px-2 py-0.5 rounded-full', urgencyConfig[referral.urgency]?.badgeClass)}>
...
function ReferralSlaPanel({ selectedSlaState }) {
  if (!selectedSlaState) {
    return null;
  }
...
function ReferralPatientInfoPanel({ referral }) {
  return (
    <div className="bg-muted border border-border rounded-lg p-4">
      ...
      <div className="space-y-2 text-sm">
        <div className="flex items-center justify-between">
          ...
          <span className="font-semibold text-foreground">{referral.patientName}</span>
        </div>
        {referral.patientMrn && (
          <div className="flex items-center justify-between">
            <span className="text-muted-foreground">MRN:</span>
            <span className="font-mono text-foreground">{referral.patientMrn}</span>
          </div>
        )}
        <div className="flex items-center justify-between">
        ...
        {referral.referred_to_specialty && (
          <div className="flex items-center justify-between">
            ...
            <span className="text-foreground">{referral.referred_to_specialty}</span>
          </div>
        )}
      </div>
    </div>
...
function ReferralDetailPanel({ referral }) {
  return (
    <div>
      ...
      <div className="space-y-3">
        ...
        {referral.clinical_summary && (
          <div>
            ...
            <p className="text-sm text-foreground whitespace-pre-wrap">{referral.clinical_summary}</p>
          </div>
        )}
      </div>
    </div>
...
function ReferralSpecialistResponsePanel({ referral }) {
  if (!referral.acceptanceNotes && !referral.decline_reason && !referral.specialist_notes) {
    return null;
  }
      ...
      <div className="space-y-3">
        {referral.acceptanceNotes && (
          <div className="bg-[oklch(0.70_0.17_155_/_0.1)] border border-[oklch(0.70_0.17_155_/_0.3)] rounded-lg p-4">
            ...
            <p className="text-sm text-foreground">{referral.acceptanceNotes}</p>
          </div>
        )}
        {referral.decline_reason && (
        ...
        {referral.specialist_notes && (referral.status === 'completed' || !referral.acceptanceNotes) && (
          <div className="bg-[oklch(0.70_0.15_230_/_0.1)] border border-[oklch(0.70_0.15_230_/_0.3)] rounded-lg p-4">
            ...
            <p className="text-sm text-foreground whitespace-pre-wrap">{referral.specialist_notes}</p>
          </div>
        )}
        {referral.recommendations && (
          <div className="bg-[oklch(0.70_0.15_230_/_0.1)] border border-[oklch(0.70_0.15_230_/_0.3)] rounded-lg p-4">
            <p className="text-sm font-heading font-medium text-[oklch(0.70_0.15_230)] mb-2">Recommendations:</p>
            <p className="text-sm text-foreground whitespace-pre-wrap">{referral.recommendations}</p>
          </div>
        )}
      </div>
    </div>
...
function ReferralTimelinePanel({ referral }) {
  return (
    <div className="bg-muted border border-border rounded-lg p-4">
      ...
      <div className="space-y-2 text-sm">
        {referral.created_at && (
          <div className="flex items-center justify-between">
            ...
            <span className="font-mono text-foreground">{format(new Date(referral.created_at), 'MMM dd, yyyy HH:mm')}</span>
          </div>
        )}
        {referral.updated_at && (
          <div className="flex items-center justify-between">
            <span className="text-muted-foreground">Last Updated:</span>
            <span className="font-mono text-foreground">{format(new Date(referral.updated_at), 'MMM dd, yyyy HH:mm')}</span>
          </div>
        )}
      </div>
    </div>
      ...
      <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto bg-card border-border">
        ...
        {selectedReferral && (
          <div className="space-y-6 py-4">
            <ReferralDetailStatusBadges referral={selectedReferral} />
            <ReferralSlaPanel selectedSlaState={selectedSlaState} />
            <ReferralPatientInfoPanel referral={selectedReferral} />
            <ReferralDetailPanel referral={selectedReferral} />
            <ReferralSpecialistResponsePanel referral={selectedReferral} />
            <ReferralTimelinePanel referral={selectedReferral} />
          </div>
        )}
      </DialogContent>
    </Dialog>
...
const ReferralSent = () => {
  ...
  if (isLoading) {
    return <SentReferralsLoadingState />;
  }
  ...
  return (
    <PageShell>
      ...
      <div className="p-4 sm:p-6 space-y-6">
        <SentReferralStatusCounts statusCounts={statusCounts} />
        ...
        <ReferralDetailDialog
          ...
          selectedSlaState={selectedSlaState}
