...
function ReferralFormHeader({ patientName, onClose }) {
  return (
...
function EmergencyReferralAlert() {
  return (
...
function ReferralDestinationSection({ formData, errors, onChange }) {
  return (
...
function ReferralUrgencySection({ formData, errors, onChange }) {
  return (
    <Card className="border-border">
...
function ReferralReasonSection({ formData, errors, onChange }) {
  return (
    <Card className="border-border">
      <CardContent className="pt-6 space-y-4">
        <div className="space-y-2">
...
function ReferralNotice({ department }) {
  return (
    <Alert>
      <AlertCircle className="size-4" />
      <AlertDescription>
        This referral will be sent to the {department || "selected"}{" "}
...
function ReferralFormFooter({ isSubmitting, onCancel, onSubmit }) {
  return (
...
function ReferralFormContent({
  ...
  return (
    <>
      <ReferralFormHeader patientName={patientName} onClose={onClose} />

      <div className="flex-1 overflow-y-auto p-6">
        <div className="space-y-6">
          {formData.urgency === "emergency" && (
            <EmergencyReferralAlert />
          )}

          <ReferralDestinationSection
            formData={formData}
            errors={errors}
            onChange={updateField}
          />

          <ReferralUrgencySection
            formData={formData}
            errors={errors}
            onChange={updateField}
          />

          <ReferralReasonSection
            formData={formData}
            errors={errors}
            onChange={updateField}
          />

          <ReferralNotice department={formData.department} />
        </div>
      </div>

      <ReferralFormFooter
        isSubmitting={createReferral.isPending || submitReferral.isPending}
        onCancel={onClose}
