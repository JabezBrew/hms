...
function AddChartHeader({ patientName, scopeContextLabel, selectedTemplate, step, onClose }) {
  return (
...
function AddChartStepIndicator({ selectedTemplate, step, onStepChange }) {
  return (
    <div className="px-6 py-3 bg-muted/30 border-b border-border">
      <div className="flex items-center gap-2">
        <button
          type="button"
          onClick={() => step > 1 && onStepChange(1)}
          className={cn(
        ...
        <button
          type="button"
          onClick={() => selectedTemplate && onStepChange(2)}
...
function AddChartTemplateSelectionStep({
  categories,
  categoryFilter,
  filteredTemplates,
  scopeContextLabel,
...
function AddChartConfigureStep({ formData, intervals, selectedTemplate, onFormDataChange }) {
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        <Select
          ...
          onValueChange={(value) => onFormDataChange((prev) => ({ ...prev, monitoring_interval: value }))}
        >
      ...
      <div className="space-y-2">
        ...
        <Textarea
          ...
          onChange={(e) => onFormDataChange((prev) => ({ ...prev, instructions: e.target.value }))}
  ...
  const filteredTemplates = useMemo(() => {
    ...
    return templates.filter((template) => {
      ...
      const isScopeCompatible = (
        template.scope_type === 'patient'
        || (template.scope_type === 'encounter' && !!encounterId)
        || (template.scope_type === 'admission' && !!admissionId)
      );
      return matchesSearch && matchesCategory && isScopeCompatible;
    });
  }, [admissionId, categoryFilter, encounterId, searchQuery, templatesData]);
