"""
Django email backends for REST API email providers.
"""
import base64
...
class BaseRestEmailBackend(BaseEmailBackend):
    """
    Shared Django EmailMessage conversion for REST email providers.
    """
    ...
    def _base_payload(self, message):
        payload = {
    ...
    def _attachment_parts(self, attachment):
        ...
        return filename, base64.b64encode(content).decode('ascii'), mimetype


class UnosendEmailBackend(BaseRestEmailBackend):
    """
    ...
    def _build_payload(self, message):
        payload = self._base_payload(message)
    ...
    def _normalize_attachment(self, attachment):
        parts = self._attachment_parts(attachment)
        if not parts:
            return None
        filename, encoded_content, mimetype = parts
...
class ResendEmailBackend(BaseRestEmailBackend):
    """
    Django email backend that sends messages through Resend.
    """
    ...
    def __init__(self, fail_silently=False, **kwargs):
        ...
        if not self.api_key:
            raise ValueError("RESEND_API_KEY setting is required")

        base_url = getattr(settings, 'RESEND_API_BASE_URL', 'https://api.resend.com')
        self.endpoint_url = f"{base_url.rstrip('/')}/emails"
        self.timeout = getattr(settings, 'RESEND_REQUEST_TIMEOUT_SECONDS', 10)

    def _send(self, message):
        """
        Send a single EmailMessage using Resend's email API.
        """
        if not message.recipients():
            return False
        ...
        raise ResendEmailError(f"Resend API returned HTTP {response.status_code}")
    ...
    def _normalize_attachment(self, attachment):
        ...
        if not parts:
            return None
        filename, encoded_content, _mimetype = parts
