...
pub async fn list_org_units(
    ...
    Ok(Json(
        state.admin_service().list_org_units(&user, query).await?,
    ))
}
...
pub async fn get_org_unit(
    ...
    Ok(Json(state.admin_service().get_org_unit(&user, id).await?))
}
...
pub async fn list_org_unit_children(
    ...
    Ok(Json(
        state
            .admin_service()
            .list_org_unit_children(&user, id, query)
            .await?,
    ))
}
...
pub async fn list_org_unit_ancestors(
    ...
    Ok(Json(
        state
            .admin_service()
            .list_org_unit_ancestors(&user, id, query)
            .await?,
    ))
}
...
pub async fn list_org_unit_descendants(
    ...
    Ok(Json(
        state
            .admin_service()
            .list_org_unit_descendants(&user, id, query)
            .await?,
    ))
}
...
pub async fn list_position_templates(
    ...
    Ok(Json(
        state
            .admin_service()
            .list_position_templates(&user, query)
            .await?,
    ))
}
...
pub async fn create_position_template(
    ...
    Ok(Json(
        state
            .admin_service()
            .create_position_template(&user, payload)
            .await?,
    ))
}
...
pub async fn list_positions(
    ...
    Ok(Json(
        state.admin_service().list_positions(&user, query).await?,
    ))
}
...
#[utoipa::path(post, path = "/api/v2/admin/staff/{id}/force-password-reset", operation_id = "postAdminStaffForcePasswordReset", tag = "admin", security(("bearerAuth" = [])), params(("id" = Uuid, Path, description = "Staff profile ID")), responses((status = 200, body = ObjectResponse<StaffListItem>), (status = 401, body = ApiErrorResponse), (status = 403, body = ApiErrorResponse), (status = 404, body = ApiErrorResponse), (status = 409, body = ApiErrorResponse)))]
